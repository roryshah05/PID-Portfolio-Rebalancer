...
def run_backtest(target_weight, Kp, Ki, Kd, minintegral, maxintegral, dt, transaction_cost, returns):
    ...
    for idx in range(len(returns_array)):
    ...
    return(u_tracker, dates, portfolio_value_tracker)
...
def run_buy_and_hold(start, end, tickers, target_weight):
    ...
    return(dates, portfolio_value_tracker)
...
if __name__ == "__main__":
    close = get_prices(tickers, start, end)
    returns = close.pct_change()
    u, dates, values = run_backtest(target_weight, Kp, Ki, Kd, minintegral, maxintegral, dt, transaction_cost, returns)
    #dates, values = run_buy_and_hold(start, end, tickers)
    #dates, values = run_threshold(target_weight, transaction_cost, start, end, tickers, threshold)
    print(values[:5])
